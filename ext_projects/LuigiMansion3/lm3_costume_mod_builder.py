...
def main():
    ...
    source_models = group_models(parse_subentries(persistent[0]))
    ...
    for target, source in TARGET_SOURCE_PAIRS:
        ...
        for target_record in global_models[target]:
            ...
            if source_record.size > target_record.size:
                raise ValueError(
                    f"slot {target} owned model allocation too small for "
                    f"{target_record.kind:04X}: {target_record.size:#x} < {source_record.size:#x}"
                )
            ...
            struct.pack_into("<I", changed[0], target_record.table_offset + 4, len(payload))
