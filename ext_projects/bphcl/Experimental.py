...
class hclSimClothPose(hkReferencedObject):
    ...
    def from_reader(cls, stream: ReadStream) -> "hclSimClothPose":
        ...
        return hclSimClothPose(**vars(base), m_name=m_name, m_positions=m_positions)
...
class hclClothData(hkReferencedObject):
    ...
    def from_reader(cls, stream: ReadStream) -> "hclClothData":
        ...
        return hclClothData(**vars(base), m_name=m_name, m_simClothDatas=m_simClothDatas, m_bufferDefinitions=m_bufferDefinitions, m_transformSetDefinitions=m_transformSetDefinitions, m_operators=m_operators, m_clothStateDatas=m_clothStateDatas, m_stateTransitions=m_stateTransitions, m_actions=m_actions, m_generatedAtRuntime=m_generatedAtRuntime, m_targetPlatform=m_targetPlatform)
...
class hkMemoryResourceContainer(hkReferencedObject):
    m_name: hkStringPtr
    m_parent: hkRefPtr # hkRefPtr[hkMemoryResourceContainer]
    m_resourceHandles: hkArray # hkArray[hkRefPtr[hkMemoryResourceHandle]]
    m_children: hkArray # hkArray[hkRefPtr[hkMemoryResourceContainer]]

    @classmethod
    def from_reader(cls, stream: ReadStream) -> "hkMemoryResourceContainer":
        base = super().from_reader(stream)
        m_name = hkStringPtr.from_reader(stream)
        m_parent = hkRefPtr.from_reader(stream, hkMemoryResourceContainer)
        m_resourceHandles = hkArray.from_reader(stream, hkRefPtr, hkMemoryResourceHandle)
        m_children = hkArray.from_reader(stream, hkRefPtr, hkMemoryResourceContainer)
