...
pub fn set_viewport_brightness(brightness: f64) -> Result<(), String> {
    crate::Settings::catch_panic_with(
        move || {
            ...
            let mut config = crate::TotkConfig::TotkConfig::safe_new(false)
                .map_err(|error| error.to_string())?;
...
pub fn update_toml_config(
    ...
    crate::Settings::catch_panic_with(
        move || {
            let mut config =
                with_document!(app_handle, documentId, app, (*app.zstd.totk_config).clone());
    ...
    crate::Settings::catch_panic_with(
        move || {
            if let rfd::MessageDialogResult::No = MessageDialog::new()
                .set_title("Warning")
                .set_description(
                    "Totkbits will be restarted, all unsaved progress will be lost. Proceed?",
                )
